...
UWAABO_HIDDEN_SIZE=16
MUNAAJPI_HIDDEN_SIZE=16
# IITEPI_HIDDEN_SIZE=32
# MEWAAJACURONE=0 #FIXME (desconocer) add to dictionary
class TSINUU_ACTOR_CRITIC(nn.Module):
    def __init__(self, alliu_size, uwaabo_size, std=0.0): # alliu is state, uwabao is action
        # --- --- --- 
        ...
        # --- ---
        self.min_std=0.05
        self.log_std = nn.Parameter(torch.ones(uwaabo_size) * (std+self.min_std))
